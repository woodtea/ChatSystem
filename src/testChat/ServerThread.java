...
public class ServerThread extends Thread {
		...
		try {
			...
			while(get_friend == false || get_group == false) {
				...
				if (type==9){
					...
					String ans = mainServer.get_id2name(from) + " " + from + 
							"<profile>" + mainServer.get_image(from) + "</profile>";
					for(int i=1;i<fri.length;i+=2) {
						String f = fri[i];
						ans += mainServer.get_id2name(f) + " " + f + 
								"<profile>" + mainServer.get_image(f) + "</profile>";
					}
	...
	void transmitMsg(Message msg) {
		 ...
		 * type==13
		 * from 发出邀请的用户
		 * to 邀请的用户+邀请其加入的群id+邀请其加入的群name
		 */
		if(type == 13) {
