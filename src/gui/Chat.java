...
public class Chat {
	volatile static JPanel blank=new JPanel();
	volatile boolean isGroup;
	volatile int id;
	volatile ImageIcon icon;
	volatile String name;
	volatile JPanel but;
	volatile JPanel mainChat;
	volatile int mainChatNum;
	volatile JPanel send;
	volatile Chat itself=this;
	@Override
	...
	void toMainUI()
	{
		...
		MainUI.chatName.setText(showName);
		MainUI.rightMid.setViewportView(mainChat);
		MainUI.nowMainChat=mainChat;
...
//        temp.ipady=100;
        ...
        sendBut.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				System.out.println("send:"+mainChatNum);
				...
				//Functions.showFriendMessage(id, "","test",itself.icon,itself.name);
